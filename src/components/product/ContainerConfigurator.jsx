...
export default function ContainerConfigurator({
  ...
  useEffect(() => {
    if (String(condition || '').toLowerCase() === 'new') {
      setGrade('IICL');
      return;
    }

    const productGrade = getProductGradeKey(container);
    setGrade(productGrade || getDefaultGrade('used'));
  ...
  const effectiveCondition =
    String(condition || '').toLowerCase() === 'new' ? 'new' : 'used';

  const isNewContainer = effectiveCondition === 'new';
  ...
  const currentTitle =
    container?.name ||
    `${isNewContainer ? 'New' : 'Used'} ${sizeOption.label} Shipping Container`;
  ...
  const handleSizeSwitch = (index) => {
    ...
    syncMatchingProduct({
      nextSizeIndex: index,
      nextCondition: effectiveCondition,
      nextGrade: effectiveGrade,
    });
  };

  const handleConditionSwitch = (nextCondition) => {
    const nextGrade = nextCondition === 'new' ? 'IICL' : getDefaultGrade('used');
  ...
  const handleGradeSwitch = (nextGrade) => {
    if (isNewContainer && nextGrade !== 'IICL') {
      return;
    }
      ...
      <div className="widget">
        ...
        <div className="section-card grade-section-card">
          ...
          <div className="grade-upgrade-grid">
            {GRADE_OPTIONS.map((g) => {
              const active = isNewContainer ? g.key === 'IICL' : grade === g.key;
              const disabled = isNewContainer && g.key !== 'IICL';

              const optionRawPrice = getRawPriceFor(
                sizeOption,
                isNewContainer ? 'IICL' : g.key,
                effectiveCondition
              );

              const difference = isNewContainer ? 0 : optionRawPrice - rawUnitPrice;
